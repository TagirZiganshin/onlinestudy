...
<div id="container" class="max-w-2xl mx-auto p-8 bg-gray-100">
  ...
  @endif
  <h1 class="text-xl font-bold mb-4">Добавить/Изменить курс</h1> <br>
  ...
  <form method="POST" action="{{ isset($training) ? route('training.update', $training) : route('training.store') }}">
    ...
    @if(isset($training))
              @method('put')
          @endif
    <input type="text" id="title" name="title" placeholder="Название курса" required class="w-full px-3 py-2 rounded border border-gray-300 mb-4" value="{{ isset($training) ? $training->title : '' }}"><br>
    <textarea id="training_description" name="description" placeholder="Описание курса" required class="w-full px-3 py-2 rounded border border-gray-300 mb-4">{{ isset($training) ? $training->description : '' }}</textarea><br><br>

    <h1 class="text-xl font-bold mb-4">Параметры</h1><br>
    <label for="start_date" class="font-bold">Начало курса:</label>
    ...
    <label for="end_date" class="font-bold">Конец курса:</label>
    ...
    <label for="estimate" class="font-bold">Длительность курса</label>
    ...
    <p class="inline-block">часов</p> <br>
    ...
    <div class="max-w-2xl mx-auto p-8 bg-gray-100" id="app">
      <h1 class="text-lg font-bold cursor-pointer hover:text-green-500" @click="toggleForm">Добавить файл</h1><br>
    ...
    <form action="{{ route('upload.file') }}" method="POST" enctype="multipart/form-data">
        ...
        <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" type="submit">Загрузить</button>
