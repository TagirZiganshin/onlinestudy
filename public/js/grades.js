...
lecturesDropdown.addEventListener("change", function () {
    ...
    fetch("/homework/fetch", {
        ...
        .then((data) => {
            ...
            let selectedOption =
                lecturesDropdown.options[lecturesDropdown.selectedIndex];
            let lectureDate = selectedOption.dataset.date;
            lectureDateElement.innerHTML =
                "Lecture date:" + "<br>" + lectureDate;
        })
        .catch((error) => {
            console.error("Error:", error);
        });
...
function updateHomeworkTable(table, data) {
    ...
    data.forEach(function (row) {
        ...
        let hasHomeworkDiv = createCheckboxDiv(
            "has_homework",
            "есть дз",
            row.id
        );
        let notWorkingDiv = createCheckboxDiv("not_working", "нет дз", row.id);
        let onTimeDiv = createCheckboxDiv("on_time", "не вовремя", row.id);
